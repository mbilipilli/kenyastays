// Shared Daraja STK callback handler. Safaricom rejects callback URLs whose
// path contains blocked words (e.g. "mpesa"), so the live route is
// /api/public/hooks/pay-callback; the legacy path stays wired for old pushes.
export async function handleDarajaCallback(request: Request): Promise<Response> {
